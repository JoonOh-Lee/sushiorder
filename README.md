...
손님이 테이블의 QR을 스캔해 주문하면, 주문 상태가 주방 디스플레이(KDS)까지 **유실 없이** 반영되는 것을 목표로 설계했습니다 — 실시간 알림(WebSocket)이 순간적으로 끊기더라도, DB가 상태의 단일 진실 공급원이라 REST 재조회로 항상 최신 상태를 다시 받아올 수 있습니다.
...
- **직접 한 것**: 문제 정의(주문 전달 누락/지연), 도메인 요구사항 및 API 계약 설계, 기술적 트레이드오프 판단(낙관적 락 + 재시도 vs 비관적 락, 재시도 한계 초과 시 자동 취소 정책, DB 커밋 후 동기 브로드캐스트 vs 별도 비동기 전달 계층 여부), 인프라 구성 방침(컨테이너 분리 단위, Ingress 라우팅 전략, 재배포 시 데이터 멱등성 정책), 실패 시나리오 정의, 코드 리뷰, 로컬 실행/테스트를 통한 동작 검증
- **AI에게 맡긴 것**: 위 결정을 바탕으로 한 실제 코드 구현 대부분

프론트엔드(별도 저장소) 쪽과는 API 스펙을 문서로 정리해 전달하는 방식으로, 양쪽에서 각각 AI 코딩 도구를 활용해 작업을 조율했습니다.

실무(SI)에서 다루지 못한 Spring Boot 4 / JPA / Kubernetes를 "AI가 짠 코드를 얼마나 정확히 검증하고 방향을 잡을 수 있는가"라는 관점에서 실제로 검증해보고 싶었던 것이 이 프로젝트를 시작한 이유 중 하나입니다.
...
- 실무(SI)에서 다루지 못한 Spring Boot 4, JPA, Kubernetes를 학습에서 끝내지 않고 실제 동작하는 서비스로 검증했습니다.
- 낙관적 락/재시도/자동 취소처럼 동시성 버그가 나면 재현이 까다로운 로직은, AI가 구현했더라도 실패 시나리오를 직접 시뮬레이션해서 검증하지 않으면 신뢰할 수 없다는 걸 체감했습니다. 스레드 10개가 재고 5개를 동시에 주문하는 테스트(`StockConcurrencyTest`)로 정확히 5건만 성공하고 나머지는 자동 취소되는지 확인했습니다.
